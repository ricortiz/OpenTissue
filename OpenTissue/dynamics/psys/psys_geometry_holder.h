...
namespace OpenTissue
{
  namespace psys
  {
    ...
    public:
      ...
      void set(std::shared_ptr<sphere_type> sphere)
      {
        m_type = SPHERE;
        m_geometry = static_cast<void*>(sphere);
      }

      void set(std::shared_ptr<plane_type> plane)
      {
        m_type = PLANE;
        m_geometry = static_cast<void*>(plane);
      }

      void set(std::shared_ptr<sdf_geometry_type> sdf)
      {
        m_type = SDF;
      ...
      void set(std::shared_ptr<aabb_tree_type> aabb_tree)
      {
        m_type = AABB_TREE;
    ...
    public:
      ...
      template<typename particle_system, typename contact_point_container>
        void dispatch(std::shared_ptr<particle_system> psys, contact_point_container & contacts)
      {
        if(m_type == SPHERE)
        {
          auto sphere = static_cast<std::shared_ptr<sphere_type>>( m_geometry );
          collision_psys_sphere(psys,*sphere,contacts);
        }
        else if(m_type == PLANE)
        {
          auto plane = static_cast<std::shared_ptr<plane_type>>( m_geometry );
          collision_psys_plane(psys,*plane,contacts);
        }
        else if(m_type == SDF)
        {
          auto sdf = static_cast<std::shared_ptr<sdf_geometry_type>>( m_geometry );
          collision_psys_sdf(psys,*sdf,contacts);
        }

        else if(m_type == AABB_TREE)
        {
          auto aabb_tree = static_cast<std::shared_ptr<aabb_tree_type>>( m_geometry );
