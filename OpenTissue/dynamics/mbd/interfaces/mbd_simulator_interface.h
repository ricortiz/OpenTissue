  ...
  namespace mbd
  {
    ...
    */
    ...
    private:
      ...
      std::shared_ptr<collision_detection> m_collision_detection;  ///< Collision Detection Engine.
      std::shared_ptr<configuration_type>  m_configuration;        ///< configuration_type, contains all the stuff that should be simulated.
      std::shared_ptr<sleepy_policy>       m_sleepy;               ///< Sleepy strategy, examines bodies and determines which ones to put to sleep.
      std::shared_ptr<stepper_policy>      m_stepper;              ///< Stepper routine (takes care of the actual simulation)

    public:

      std::shared_ptr<stepper_policy>       get_stepper()       { return m_stepper; }
      std::shared_ptr<stepper_policy> const get_stepper() const { return m_stepper; }
      std::shared_ptr<sleepy_policy>        get_sleepy()        { return m_sleepy;  }
      std::shared_ptr<sleepy_policy>  const get_sleepy()  const { return m_sleepy;  }

      std::shared_ptr<collision_detection> const get_collision_detection() const
      {
      ...
      std::shared_ptr<collision_detection> get_collision_detection()
      {
        assert(m_collision_detection || !"SimulatorInterface::get_collision_detection(): null pointer, did you forget to invoke init?");
        return m_collision_detection;
      }

      std::shared_ptr<configuration_type>  const get_configuration()       const
      {
        assert(m_configuration || !"SimulatorInterface::get_configuration(): null pointer, did you forget to invoke init?");
        return m_configuration;
      }

      std::shared_ptr<configuration_type> get_configuration()
      {
        assert(m_configuration || !"SimulatorInterface::get_configuration(): null pointer, did you forget to invoke init?");
        return m_configuration;
      }
    ...
    public:
      ...
      {
        m_collision_detection = std::make_shared<collision_detection>();
        m_sleepy              = std::make_shared<sleepy_policy>();
        m_stepper             = std::make_shared<stepper_policy>();
      }
