...
//
#include "setup_ball_joint.h"

void setup_ball_joint(Data & data)
{
  real_type const friction    = 0.25;
  real_type const restitution = 0.15;
  real_type const timestep    = 0.01;  // We assume that timestep is 0.01
  ...
  data.m_bodies[2].set_velocity(vector3_type(0,0,1));
  ...
  data.m_reach_cone.add_cone( vector3_type(  0,-1, 0.25 ) );
  data.m_reach_cone.add_cone( vector3_type( -1, 0, 0.25 ) );
  data.m_reach_cone.add_cone( vector3_type(  0, 1, 0.25 ) );
  data.m_reach_cone.add_cone( vector3_type(  1, 0, 0.25 ) );
  data.m_ball.set_reach_cone(data.m_reach_cone);

  data.m_configuration.add(&data.m_ball);
